...
func (c *HomeController) Prepare() {
	...
	model.Db.Where("pid=? and is_nav=?", 0, 1).Find(&booksType)
	//子导航
...
func GetList(cid, id string) (bookList BookList, bookArticle BookArticle) {
	...
	c.OnHTML("html", func(e *colly.HTMLElement) {
		...
		if res.BookLastAt != bookLastTime {
			model.Db.Model(&models.Books{}).Where("id=?", res.Id).Update("book_last_at", bookLastTime)
		}
		if res.BookAuthor == "" {
			model.Db.Model(&models.Books{}).Where("id=?", cid).Update("book_author", bookAuthor)
		}
		if res.BookNewChapter != lastTitle {
			model.Db.Model(&models.Books{}).Where("id=?", res.Id).Update("book_new_chapter", lastTitle)
		}
